| ord (norsk) | ord (s.g)   | ord (p.l)   | gender |
| ----------- | ----------- | ----------- | ------ |
| brødet      | il pane     | i pani      | m      |
| vannet      | l'acqua     | le acque    | f      |
| huset       | la casa     | le case     | f      |
| bilen       | la macchina | le macchine | f      |
| boken       | il libro    | i libri     | m      |
| døren       | la porta    | le porte    | f      |
| vinduet     | la finestra | le finestre | f      |
| bordet      | il tavolo   | i tavoli    | m      |
| stolen      | la sedia    | le sedie    | f      |
| nøkkelen    | la chiave   | le chiavi   | f      |

| ord (norsk) | ord (s.g)   | ord (p.l)   | gender |
| ----------- | ----------- | ----------- | ------ |
| melken      | il latte    | i latti     | m      |
| kaffen      | il caffè    | i caffè     | m      |
| telefonen   | il telefono | i telefoni  | m      |
| klokken     | l'orologio  | gli orologi | m      |
| sengen      | il letto    | i letti     | m      |
| kjøkkenet   | la cucina   | le cucine   | f      |
| gaten       | la strada   | le strade   | f      |
| byen        | la città    | le città    | f      |
| hunden      | il cane     | i cani      | m      |
| katten      | il gatto    | i gatti     | m      |

| ord (norsk) | ord (s.g) | ord (p.l)  | gender |
| ----------- | --------- | ---------- | ------ |
| eplet       | la mela   | le mele    | f      |
| vinen       | il vino   | i vini     | m      |
| skoen       | la scarpa | le scarpe  | f      |
| jakken      | la giacca | le giacche | f      |
| skolen      | la scuola | le scuole  | f      |
| jobben      | il lavoro | i lavori   | m      |
| moren       | la madre  | le madri   | f      |
| faren       | il padre  | i padri    | m      |
| dagen       | il giorno | i giorni   | m      |
| natten      | la notte  | le notti   | f      |

| ord (norsk)  | ord (s.g)     | ord (p.l)    | gender |
| ------------ | ------------- | ------------ | ------ |
| barnet       | il bambino    | i bambini    | m      |
| maten        | il cibo       | i cibi       | m      |
| hånden       | la mano       | le mani      | f      |
| øyet         | l'occhio      | gli occhi    | m      |
| kirken       | la chiesa     | le chiese    | f      |
| restauranten | il ristorante | i ristoranti | m      |
| toget        | il treno      | i treni      | m      |
| stranden     | la spiaggia   | le spiagge   | f      |
| blomsten     | il fiore      | i fiori      | m      |
| treet        | l'albero      | gli alberi   | m      |
